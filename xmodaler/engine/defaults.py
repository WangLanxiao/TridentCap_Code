...
class DefaultTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        # inputs={}
        # inputs['IDS'] = torch.tensor(5).cuda()
        # inputs['ATT_FEATS'] = torch.randn(1,37, 2048).cuda()
        # inputs['ATT_MASK'] = torch.ones(1,37).cuda()
        # inputs['TOKENS_MASK'] = torch.ones(1,27).cuda()
        # inputs['GLOBAL_FEATS'] = torch.randn(1,2048).cuda()
        # inputs['ATT_FEATS_LOC'] = torch.randn(1,37, 5).cuda()
        # inputs['MID_G_TOKENS_TYPE'] = torch.stack([torch.tensor([1]*27)]).cuda()
        # inputs['MID_G_TOKENS_IDS'] = torch.stack([torch.tensor([1]*27)]).cuda()
        # inputs['MID_G_TARGET_IDS'] =  torch.stack([torch.tensor([1]*27)]).cuda()
        # inputs['G_TOKENS_TYPE'] = torch.stack([torch.tensor([1] * 27)]).cuda()
        # inputs['G_TOKENS_IDS'] = torch.stack([torch.tensor([1] * 27)]).cuda()
        # inputs['G_TARGET_IDS'] = torch.stack([torch.tensor([1] * 27)]).cuda()
        # inputs['STYLE_TOKEN'] = torch.tensor([[1]]).cuda()
        # inputs['SS_PROB'] = 0.0
        #
        ...
        self.val_data_loader = None   #wlx
        self.test_data_loader = self.build_test_loader(cfg)
        # self.test_data_loader = None   #wlx
        ...
        if comm.get_world_size() > 1:
            model = DistributedDataParallel(
                model, find_unused_parameters=cfg.FIND_UNUSE,
    ...
    def _write_metrics(
        ...
        if comm.is_main_process():
            ...
            if 'Style Loss' in metrics_dict:
                storage.put_scalar("{}style_loss".format(prefix), metrics_dict['Style Loss'])
    ...
    def test(cls, cfg, model, test_data_loader, evaluator, epoch):
        ...
        with torch.no_grad():
            for data in tqdm.tqdm(test_data_loader):
                ...
                if kfg.IDS in data:       #one stage
                    ids = data[kfg.IDS]
                    types = data['STYLE_TOKEN']
                    if cfg.INFERENCE.GENERATION_MODE == True:
                        res = model(data, use_beam_search=True, output_sents=True)
                    else:
                        res = model(data)
                    outputs = res[kfg.OUTPUT]
                    style_classes = res['STYLE_CLASS']
                    for id, type, output,style_class in zip(ids, types, outputs, style_classes):
                        results.append(
                            {cfg.INFERENCE.ID_KEY: int(id), 'TYPE_gt': int(type), 'TYPE_predict': int(style_class), cfg.INFERENCE.VALUE: output})
                else:
                    if kfg.COCO_IDS in data:
                        ids = data[kfg.COCO_IDS]
                    elif kfg.CROWD_IDS in data:
                        ids = data[kfg.CROWD_IDS]
                    if cfg.INFERENCE.GENERATION_MODE == True:
                        res = model(data, use_beam_search=True, output_sents=True)
                    else:
                        res = model(data)
                    outputs = res[kfg.OUTPUT]
                    reoutputs = res[kfg.REOUTPUT]
                    for id, output, reoutput in zip(ids, outputs, reoutputs):
                        results.append({cfg.INFERENCE.ID_KEY: int(id), cfg.INFERENCE.VALUE: reoutput, 'output_wogt': output})

        if evaluator is not None:
            eval_res = evaluator.eval(results, epoch)
        else:
            json.dump(results,open(os.path.join(cfg.OUTPUT_DIR, str(epoch) +'.json'), "w"))
            eval_res = ''
        model.train()
        return eval_res
