...
class BeamSearcherMerge(DecodeStrategy):
    ...
    def _forward(self, batched_inputs, model):
        ...
        for t in range(self.max_seq_len):
            ...
            Style_hidd.append(inputs['G_HIDDEN_STATES'][-1])
        ...
        Style_hidd = torch.stack(Style_hidd, 1).mean(-2).reshape(batch_size, beam_size,-1)
        Style_hidd = torch.gather(Style_hidd, 1, sort_idxs.expand(batch_size, beam_size,Style_hidd.shape[-1]))

        outputs = outputs.contiguous()[:, :out_size]
        log_probs = log_probs.contiguous()[:, :out_size]
        Style_hidd = Style_hidd.contiguous()[:, :out_size,:]
        if out_size == 1:
            outputs = outputs.squeeze(1)
            log_probs = log_probs.squeeze(1)
            Style_hidd = Style_hidd.squeeze(1)
