...
class BeamSearcher(DecodeStrategy):
    ...
    def _forward(self, batched_inputs, model):
        ...
        # if kfg.IMG_INPUT in batched_inputs:
        #     backbone_out = model.backbone(inputs['IMG_INPUT'])
        #     inputs.update(backbone_out)
            # for idx,sub in enumerate(nixinputs['IDS']):
            #     path = '/data1/wlx/project/open_source_dataset/COCOandCrowd/features/crowd_human_swin/'+str(sub)+'.npy'
            #     vvv = backbone_out['ATT_FEATS'][idx].cpu().numpy()
            #     np.save(path, vvv)
