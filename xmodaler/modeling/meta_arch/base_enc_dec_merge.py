...
class BaseEncoderDecoderMerge(nn.Module, metaclass=ABCMeta):
    @configurable
    def __init__(
        self,
        *,
        ...
        super(BaseEncoderDecoderMerge, self).__init__()
    ...
    def add_config(cls, cfg, tmp_cfg):
        ...
        add_vl_merge_config(cfg, tmp_cfg)
    ...
    def preprocess_batch(self, batched_inputs):
        ...
        if 'MID_'+ kfg.G_TOKENS_IDS in batched_inputs[0]:
            mid_g_tokens_ids = [x['MID_'+kfg.G_TOKENS_IDS] for x in batched_inputs]
            mid_g_tokens_ids, tmasks = pad_tensor(mid_g_tokens_ids, padding_value=0, use_mask=True)
            ret.update( {'MID_'+ kfg.G_TOKENS_IDS: mid_g_tokens_ids} )

        if 'MID_'+ kfg.G_TARGET_IDS in batched_inputs[0]:
            mid_g_target_ids = [x['MID_'+kfg.G_TARGET_IDS] for x in batched_inputs]
            mid_g_target_ids = pad_tensor(mid_g_target_ids, padding_value=-1, use_mask=False)
            ret.update({ 'MID_'+ kfg.G_TARGET_IDS: mid_g_target_ids })

        if 'MID_'+ kfg.G_TOKENS_TYPE in batched_inputs[0]:
            mid_g_tokens_type = [x['MID_'+kfg.G_TOKENS_TYPE] for x in batched_inputs]
            mid_g_tokens_type = pad_tensor(mid_g_tokens_type, padding_value=1, use_mask=False)
            ret.update({ 'MID_'+ kfg.G_TOKENS_TYPE: mid_g_tokens_type })

        if 'STYLE_TOKEN' in batched_inputs[0]:
            style_tokens = [x['STYLE_TOKEN'] for x in batched_inputs]
            style_tokens = torch.stack(style_tokens,-1).unsqueeze(1)
            ret.update({'STYLE_TOKEN': style_tokens })
        ...
        ######################################################################################
        if kfg.SEQ_PER_SAMPLE_NET1 in batched_inputs[0]:
            batch_size, max_feats_num, feats_dim = vfeats.size()[0:3]
            repeat_num = batched_inputs[0][kfg.SEQ_PER_SAMPLE_NET1].item()
            ...
            if 'STYLE_TOKEN' in batched_inputs[0]:
                style_tokens = style_tokens.expand(batch_size, repeat_num).reshape(-1,1)
                ret.update({'STYLE_TOKEN': style_tokens})
        ...
        if kfg.STYLE in batched_inputs[0]:
            ids_type = [x[kfg.STYLE]  for x in batched_inputs ]
            if kfg.SEQ_PER_SAMPLE_NET1 in batched_inputs[0]:
                ids_type = np.repeat(np.expand_dims(ids_type, axis=1), repeat_num, axis=1).flatten()
            ret.update({ kfg.STYLE: ids_type })

        if kfg.STYLE_ID in batched_inputs[0]:
            ids_type_id = [x[kfg.STYLE_ID]  for x in batched_inputs ]
            if kfg.SEQ_PER_SAMPLE_NET1 in batched_inputs[0]:
                ids_type_id = np.repeat(np.expand_dims(ids_type_id, axis=1), repeat_num, axis=1).flatten()
            ret.update({ kfg.STYLE_ID: ids_type_id })
